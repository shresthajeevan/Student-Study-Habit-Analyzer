import {
  ...
  function Dashboard({ sessions }) {
    ...
    const strongSubjects = subjectTimeData.slice(0, 3);
    const weakSubjects = subjectTimeData.slice(-3).reverse();
    ...
    const SubjectList = ({ subjects, color, icon: Icon, title, subtitle }) => (
      <div className="bg-gradient-to-br from-white to-blue-50/30 rounded-xl p-6 border border-gray-200">
        ...
        {subjects.length > 0 ? (
            ...
            {subjects.map((subject, i) => (
              <li key={subject.subject} className="flex items-center justify-between p-3 bg-white/80 rounded-lg border border-gray-100">
                <div className="flex items-center gap-3">
                  <span className={`w-7 h-7 ${color} text-white rounded-full flex items-center justify-center text-sm font-bold`}>
                    {i + 1}
                  </span>
                  <span className="font-semibold text-[#111827]">{subject.subject}</span>
                </div>
                <span className={`px-2.5 py-1 ${color.replace('bg-', 'bg-').replace('-500', '-100')} ${color.replace('bg-', 'text-').replace('-500', '-700')} rounded-full text-sm font-semibold`}>
                  {subject.hours}h
                </span>
              </li>
            ))}
          ...
          <p className="text-[#4b5563] text-center py-4 text-sm">No data yet</p>
        )}
      </div>
    );
    ...
    <ResponsiveContainer width="100%" height={300}>
      <PieChart>
        <Pie
          ...
          dataKey="minutes"
          nameKey="subject"
          labelLine={false}         // remove lines to avoid clutter
          label={({ cx, cy, midAngle, innerRadius, outerRadius, percent, index }) => {
            const RADIAN = Math.PI / 180;
            const radius = innerRadius + (outerRadius - innerRadius) * 1.2; // push labels outside
