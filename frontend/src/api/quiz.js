...
export async function getAllQuizzes(subject = null) {
  ...
  if (!res.ok) throw new Error((await res.json()).message || "Failed to fetch quizzes");
  return res.json();
}
...
export async function getQuizResults(quizId) {
  const res = await fetch(`${BASE_URL}/${quizId}/results`, { method: "GET", credentials: "include" });
  if (!res.ok) throw new Error((await res.json()).message || "Failed to fetch results");
  return res.json();
}
...
export async function deleteQuiz(id) {
  ...
  if (!res.ok) throw new Error((await res.json()).message || "Failed to delete quiz");
  return res.json();
}
